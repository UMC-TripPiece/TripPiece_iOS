...
class VideoLogViewController: UIViewController {
            ...
            do {
                ...
                logVideo(memoTextView.text ?? "", video: videoData) { result in
                    ...
                    case .failure(let error):
                        ...
                        let alert = UIAlertController(title: "오류", message: "비디오 업로드에 실패했습니다.", preferredStyle: .alert)
    ...
    func logVideo(_ description: String, video: Data, completion: @escaping (Result<Any, Error>) -> Void) {
        guard
            !description.isEmpty,
            !video.isEmpty
        else {
            let error = NSError(domain: "Upload Memo Error", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid inputs for memo or video"])
        ...
        OngoingLogManager.postTravelVideo(data) { isSuccess, response in
            if isSuccess {
                ...
                let error = NSError(domain: "Log Video Error", code: -1, userInfo: [NSLocalizedDescriptionKey: "Log video update failed."])
                completion(.failure(error))
            }
        }
    }
...
extension VideoLogViewController: UITextViewDelegate {
    ...
    func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        ...
        return updatedText.count <= 100 // 글자 수 제한 30자
    }
